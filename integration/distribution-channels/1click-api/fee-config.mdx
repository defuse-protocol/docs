...
- **Constraints:** Fee range: `0` to `10,000` (where `10,000` = 100%)

<Info>
A **50/50 revenue share** applies by default — half of the `fee` amount goes to your `recipient` address and half goes to the 1Click protocol address. For example, `"fee": 10` means 5 bps to you and 5 bps to 1Click. This split is negotiable — contact us through the [Partner Dashboard](https://partners.near-intents.org/) for custom terms.
</Info>
