...
The [Quickstart](./quickstart.mdx) tutorial explains how to setup an [example AMM Solver](https://github.com/defuse-protocol/near-intents-amm-solver),
...
<Warning>
The WebSocket endpoint requires an API key. Pass it as a Bearer token in the connection headers. See the [Quickstart](/integration/market-makers/quickstart) for how to obtain one.
</Warning>
...
Every intent requires a [unique nonce](/integration/verifier-contract/intent-types-and-execution#nonce-structure). The example solver uses a deterministic approach — it hashes the current token reserves with SHA-256, so the nonce only changes when the solver's balances change (i.e. after a trade settles):
